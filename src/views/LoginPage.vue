...
const handleSubmit = async () => {
  ...
  if (!model.value.username) {
    msgError.value = 'Account name cannot be left blank';
    return;
  }

  if (!model.value.password) {
    msgError.value = 'Password cannot be left blank';
    return;
  }
  ...
  try {
    ...
    console.log("username:  " +model.value.username+ "password: "+ model.value.password)
    if (response.data && response.data.code === 0 && response.data.result && response.data.result.token) {
      const token = response.data.result.token;
      localStorage.setItem('authToken', token);
      console.log(token)
      console.log("đăng nhập thành công")
      await fetchUserInfoAndCart(token);
      msgSuccess.value = 'Log in successfully';
      router.push({ name: 'home' });
    } else if (response.data && response.data.code === 1003) {
      msgError.value = 'Account or password is incorrect';
    } else {
      msgError.value = 'Login failed. Please try again later.';
    }
  } catch (error) {
...
const fetchUserInfoAndCart = async (token) => {
  try {
    const userInfoResponse = await axios.get('http://localhost:8080/users/myInfo', {
      headers: { Authorization: `Bearer ${token}` },
    });
    localStorage.setItem('_USER_ID', userInfoResponse.data.result.id);
    const cartCountResponse = await axios.get(`http://localhost:8080/cart/count/${userInfoResponse.data.result.id}`);
    _COUNT_CART.value = cartCountResponse.data.count;
  } catch (error) {
    console.error('Error fetching user info or cart count:', error);
  }
};
          ...
          <div class="col-md-6 col-lg-5">
            <div class="card">
              <div class="card-body p-4 p-md-5">
                ...
                <h6 class="card-subtitle mb-2 text-body-secondary">New to Meow Shop? <RouterLink class="text-primary" :to="{name: 'signup'}">Sign up</RouterLink></h6>
                <div class="card-text">
                ...
                <div class="mt-4 d-flex">
                  <button class="btn btn-primary rounded-5 w-100" @click="handleSubmit">LOG IN</button>
                </div>
              </div>
            </div>
          </div>
